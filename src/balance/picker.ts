 ...
 * 三选一规则（纯数据 + 抽取规则）
 *
 * 一局的选择序列：开局挑 1 个村民 → 第 2、3 波各再挑 1 个（共 3 人）→
...
/**
 * 三选一出现在这些波次之前。加上开局首次选人，一局共 10 次选择。
 *
 * 第 2、3 波连续给村民，是因为一个人谈不上阵容；第 4 波起全部是改装件。
 * 之后隔一波给一次，把节奏让回给观战。
 */
export const PICK_WAVES: readonly number[] = [2, 3, 4, 5, 6, 8, 10, 12, 14];

/** 这些波次发的是村民，其余发改装件 */
export const RECRUIT_WAVES: readonly number[] = [2, 3];
