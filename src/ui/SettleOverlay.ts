...
export class SettleOverlay extends PIXI.Container {
  ...
  constructor(onReplay: () => void, onDouble: () => Promise<boolean>) {
    super();
  ...
  show(
    ...
    const panelW = 620;
    const panelH = opts.canDouble ? 640 : 580;
    ...
    const rowH = 78;
    const listY = py + (core && core.mods.length > 0 ? 118 : 100);
    ...
    scrapLine.text = `本局攒了 ${opts.earned} · 花了 ${opts.spent} · 剩 ${opts.scrap}`;
    ...
    if (opts.canDouble) {
      ...
      dub.addChild(dbg, dl);
      dub.position.set(375, scrapY + 96);
      this.addChild(dub);
      bindPointerTap(dub, async () => {
        ...
        this._busy = false;
        if (!ok) return;
        scrapLine.text = `下局开场带 ${doubled} 废品`;
        dub.visible = false;
        btnY = scrapY + 96;
        replay.position.y = btnY;
      });
      btnY = scrapY + 186;
    }
    ...
    goldBtn(bg, -150, -40, 300, 80);
    const label = text(28, GOLD, true);
    label.anchor.set(0.5);
    label.text = '再来一局';
    replay.addChild(bg, label);
    replay.position.set(375, btnY);
