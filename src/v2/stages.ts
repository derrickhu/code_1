...
/** 3 路 × 4 格。12 个格子，满级只给 8 个人上，空的 4 格就是取舍 */
 ...
 *
 * 和 CELL_SPAN 一起决定了「后排能不能帮上忙」，别单独改其中一个。
 * 第一版是 SPAWN_GAP=4 / CELL_SPAN=2，战场 12 格长而射程只有 1~5，
 * 结果敌人被 cell 0 的人挡在 pos 3.5，**cell 2 和 cell 3 永远打不到任何东西**，
 * 而布阵策略恰好把「打」位放在 cell 2 —— 上场人数从 3 涨到 8 几乎没换来输出，
 * 第 6 章往后必然撞墙。
 *
 * 现在的口径：四格分别落在 pos 2/3/4/5，敌人挡点在 pos 1.5，
 * 四个定位的射程都刚好够到挡点（见 villagers.ROLE_BASE 的 range）。
 */
...
/** 一波隔多久放下一波。放在这儿是因为它属于关卡节奏，星评的 par 时间要用 */
export const WAVE_GAP_MS = 10_000;

/** 最后一只出场后再给多久算「清得利索」。par 时间 = 最后出场 + 这个 */
