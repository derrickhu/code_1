/**
 * v2 数值报表：npm run sim2
 *
 * 旧的 npm run sim 打的是上一版（改装件构筑、失败=队灭、15 波一图），
 * 那套口径已经被这一版推翻了，两个工具**并存但不互通**：
 * 旧的会继续绿着直到旧引擎下线，别拿它的阈值来判这一版。
 *
 ...
 */
import { LANE_COUNT, CELL_COUNT, STAGE_COUNT, stageEnemyCount } from '../src/v2/stages';
import { assertWeights, DAILY_PELLETS, expectedPerDay } from '../src/v2/stall';
import {
  SQUAD_CAP_MAX, VILLAGE_LV_MAX, villageCumExp, villageMul,
} from '../src/v2/village';
import { LANE_NAME, VILLAGERS, assertRosterComplete } from '../src/v2/villagers';
import { simulate, sweepStages, sweepStats, type SimResult } from '../src/v2/sim';
